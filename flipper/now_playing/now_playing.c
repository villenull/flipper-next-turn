...
int32_t now_playing_app(void *ctx) {
  ...
  if (!startup_error) {
    ...
    if (np_ble_valid(a->profile)) {
      ...
      /* The GAP layer reports set_discoverable failures only in disabled
       * logs; confirm the controller is actually advertising or connected
       * before presenting "Waiting for phone". */
