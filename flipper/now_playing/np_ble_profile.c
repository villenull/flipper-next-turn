...
static void config(GapConfig *c, FuriHalBleProfileParams ctx) {
  ...
  c->mac_address[2] += 2;
  /* aci_gap_set_discoverable auto-adds Flags (3) + TX Power (3) on top of
   * our data, so name + 128-bit UUID blocks must fit 25 bytes: with the
   * 18-byte UUID block the name field (2 + N) leaves N <= 5. A fixed short
   * name also works with custom Flipper names that contain no "Flipper"
   * substring to rewrite. */
  snprintf(c->adv_name, sizeof(c->adv_name), "NP%02X", c->mac_address[5]);
}
