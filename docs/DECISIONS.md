...
## Fixed short BLE advert name and advertising self-check (FAP 1.2)
Legacy advertising payload is 31 bytes; the 128-bit Now Playing service UUID occupies 18, leaving 10 bytes for the name AD structure (name <= 8 chars). The previous `Flipper→NP` rewrite depended on the stock device name and failed on custom names (observed: `Betfoyet`), pushing the payload to the limit with `aci_gap_set_discoverable` failures visible only in disabled logs. Use a fixed `NP %02X%02X` advert name derived from the offset MAC tail (7 chars, 30/31 total), and poll the exported `furi_hal_bt_is_active()` for 2 s after starting advertising, showing `Advertising failed` rather than a false `Waiting for phone`. Service UUIDs, protocol, MAC offset (+2), pairing method and key-storage path are unchanged.

## Advert budget is 25 bytes, not 31 (FAP 1.2 correction)
`aci_gap_set_discoverable` automatically appends Flags and TX Power AD structures, so caller-supplied name + service-UUID blocks must fit 25 bytes. With the fixed 18-byte 128-bit UUID block the complete local name is limited to 5 chars; use `NP%02X` (last MAC byte). Device logs proved rejection (`set_discoverable failed 146` = `BLE_STATUS_INVALID_PARAMS`); the app-layer `furi_hal_bt_is_active()` check cannot catch this because the GAP layer advances its state even when the controller rejects the parameters.

## Nav Turns GPS app (standalone sibling)
User requested a standalone Flipper app showing Google Maps turn-by-turn: next road name, turn type, distance. Chosen design (user-confirmed): phone reads the Maps navigation notification (same listener pattern as the media helper, Maps package only, ongoing notifications only) and a **new standalone APK** (`io.github.flippernowplaying.nav`) so either helper installs alone.
New `NAV/1` protocol reuses the proven FNP1/v1 frame envelope with its own BLE service (`04b78bcf-…`, RX `4bb67121-…`, TX `e03b9f24-…`), message types 1/2 hello, 80 snapshot, 81 ack, 82 refresh-request, 96/97 heartbeat, 126/127 error/close. Snapshot carries epoch/revision, turn enum 0–14, flags, and three ≤64-char ASCII strings. Reference codec, 14 frozen vectors and a 9-case parser spec live under `protocol/`; C and Kotlin ports must match them.
Flipper advert name `NV%02X` (4 chars, 24/25 bytes) applies the learned 25-byte caller budget; MAC offset +4 keeps a distinct identity from Now Playing (+2). Right-press next-turn preview is **skipped**: the notification feed exposes only the current maneuver, so there is no second turn to show; OK re-requests a full snapshot instead. Long Back exits, default profile/keys restore as usual.
